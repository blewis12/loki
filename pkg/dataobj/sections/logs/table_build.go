...
func buildTable(buf *tableBuffer, pageSize int, compressionOpts dataset.CompressionOptions, records []Record) *table {
	...
	for i, record := range records {
		...
		record.Metadata.Range(func(l labels.Label) {
			...
			metadataBuilder := buf.Metadata(l.Name, pageSize, compressionOpts)
			_ = metadataBuilder.Append(i, dataset.ByteArrayValue(unsafeSlice(l.Value, 0)))
		})
	}
